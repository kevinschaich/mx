...
export class HIDReader extends EventEmitter {
  ...
  interpretChange(change) {
    ...
    if (scrollWheels[index]) {
      ...
      // Detect scrolls immediately when value becomes non-zero (not when it returns to zero)
      if (curr >= 1 && curr <= 127 && prev !== curr) {
      ...
      } else if (curr >= 128 && curr <= 255 && prev !== curr) {
    ...
    if (prev === 0 && curr >= 1 && curr <= 255) {
      // This looks like a scroll wheel
      if (curr >= 1 && curr <= 127) {
        // Clockwise
        if (!this.scrollPositions[index]) this.scrollPositions[index] = 0;
